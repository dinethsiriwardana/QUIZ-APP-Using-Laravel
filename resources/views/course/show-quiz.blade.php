        ...
        @if (isset($totalMarks))
            <div class="row d-xl-flex justify-content-xl-center align-items-xl-center"
                ...
                <div class="col">
                    ...
                    <h1 style="color: #01c38d;margin-bottom: 0px;text-align: center;">
                        {{ $totalMarks }}/{{ $quizQuestions->count() }}</h1>
                </div>
            </div>
        @endif
        <div class="container d-flex d-xl-flex flex-column justify-content-xl-center align-items-xl-center"
            style="margin-top: 50px;margin-bottom: 50px;margin-right: 0px;margin-left: 0px;max-width: 100%;width: 80%;">
                    ...
                    @foreach ($quizQuestions as $key => $quizQuestion)
                        <div class="quizblock {{ $key == 0 ? '' : 'd-none' }}" id="question_{{ $key }}">
                            <div class="col d-xl-flex justify-content-xl-end">
                                <h1 style="color: var(--bs-white);font-size: 32px;">{{ $key + 1 }} /
                                    {{ $quizQuestions->count() }}</h1>
                            </div>
                            ...
                            <div class="col" style="font-size:25px">
                                <div class="form-check" style="padding-bottom: 20px;">
                                    <input class="form-check-input" type="radio" required
                                        ...
                                        {{ isset($totalMarks) && $quizQuestion->quiz_correct == 1 ? 'checked' : '' }}>
                                    <label class="form-check-label" for="formCheck-1"
                                        style="color: {{ isset($totalMarks) && $quizQuestion->quiz_correct == 1 ? '#01c38d' : 'var(--bs-white)' }};">{{ $quizQuestion->answer_1 }}</label>
                                </div>
                                <div class="form-check" style="padding-bottom: 20px;">

                                    <input class="form-check-input" type="radio" required
                                        name="{{ $quizQuestion->quiz_no }}" value="2"
                                        {{ isset($totalMarks) && $quizQuestion->quiz_correct == 2 ? 'checked' : '' }}>
                                    <label class="form-check-label" for="formCheck-4"
                                        style="color: {{ isset($totalMarks) && $quizQuestion->quiz_correct == 2 ? '#01c38d' : 'var(--bs-white)' }};">{{ $quizQuestion->answer_2 }}</label>
                                </div>
                                <div class="form-check" style="padding-bottom: 20px;">

                                    <input class="form-check-input" type="radio" required
                                        name="{{ $quizQuestion->quiz_no }}"value="3"
                                        {{ isset($totalMarks) && $quizQuestion->quiz_correct == 3 ? 'checked' : '' }}>
                                    <label class="form-check-label" for="formCheck-3"
                                        style="color: {{ isset($totalMarks) && $quizQuestion->quiz_correct == 3 ? '#01c38d' : 'var(--bs-white)' }};">{{ $quizQuestion->answer_3 }}</label>
                                </div>
                                <div class="form-check" style="padding-bottom: 20px;">

                                    <input class="form-check-input" type="radio" required
                                        name="{{ $quizQuestion->quiz_no }}"value="4"
                                        {{ isset($totalMarks) && $quizQuestion->quiz_correct == 4 ? 'checked' : '' }}>
                                    <label class="form-check-label" for="formCheck-2"
                                        style="color: {{ isset($totalMarks) && $quizQuestion->quiz_correct == 4 ? '#01c38d' : 'var(--bs-white)' }};">{{ $quizQuestion->answer_4 }}</label>
                                </div>
                            </div>
            ...
            {{-- @if (!isset($totalMarks)) --}}
            <div class="col-auto">
                <button class="btn btn-primary d-block icon-button w-100" type="button" {{-- onclick="location.href = '{{ route('course', ['id' => $course_id] }}';" --}}
                    onclick="location.href = '/courses/{{ $course_id }}';"
                    style="width: 250px;max-width: 250px;background: #01c38d;border-style: none;"
                    data-bs-target="#target"><i class="fa fa-home"></i><span>Go Back</span>
                </button>
            </div>
            {{-- @endif --}}
        </div>
